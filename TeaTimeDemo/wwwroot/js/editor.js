...
function initializeFillInBlankIndices() {
    ...
    for (var i = 0; i < window.questionIndex; i++) {
        ...
        optionContainers.each(function () {
            ...
            // 调用更新选项文本的函数，确保选项描述正确显示填空占位符
            // updateOptionTextWithBlanks();
        });
        // **在这里调用 updateQuestionTextWithBlanks 函数**
...
function updateOptionTextWithBlanks(questionIndex, optionIndex) {
    ...
    // **移除選項文本中已存在的填空佔位符**
    optionText = optionText.replace(/\$\{填空\d+\}\$/g, '');

    var fillInBlanks = optionContainer.find('.fill-in-blank');

    // 收集未被刪除的填空，並重新編號
    var placeholders = [];
    var blankCounter = 1; // 重新編號的起始值
    ...
    fillInBlanks.each(function () {
        ...
        var position = parseInt($(this).find('input[name$=".Position"]').val()) || 0;
        // 使用重新編號的 blankCounter
        var placeholder = '${填空' + blankCounter + '}$';
        placeholders.push({ position: position, placeholder: placeholder });
        blankCounter++;
    });
    ...
    // **更新填空的顯示編號**
    // 重新編號填空的顯示部分（例如，h5 元素中的 "填空 1"）
    var newBlankNumber = 1;
    fillInBlanks.each(function () {
        if ($(this).find('input[name$=".IsDeleted"]').val() === 'true') {
            return; // 跳過已刪除的填空
        }
        $(this).find('h5').text('填空 ' + newBlankNumber);
        newBlankNumber++;
    });
}
